...
export function MarketStrip() {
  ...
  const items = [
    { icon: Zap, label: "Live", value: formatCurrency(stats.liveNotional), color: "text-gold" },
    { icon: TrendingUp, label: "Active", value: formatNumber(stats.activeCount), color: "text-emerald-800" },
    { icon: TrendingDown, label: "Expired", value: formatNumber(stats.expiredCount), color: "text-stone-600" },
  ...
  return (
    <div className="overflow-hidden border-b border-stone-200/80 bg-white/80">
      <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-4 px-6 py-1.5">
        ...
        <p className="truncate text-[10px] text-stone-500">{uploadState}</p>
      </div>
    </div>
  );
}
